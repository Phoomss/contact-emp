...
const createContract = async (req, res) => {
  ...
  if (!number) {
    return res
      .status(400)
      .json({ message: "Please fill out the number field" });
  }

  if (!start_date) {
    return res
      .status(400)
      .json({ message: "Please fill out the start_date field" });
  }

  if (!end_date) {
    return res
      .status(400)
      .json({ message: "Please fill out the end_date field" });
  }

  if (!company_id) {
    return res
      .status(400)
      .json({ message: "Please fill out the company field" });
  }
...
const getContractWithAllParams = async (req, res) => {
  ...
  if (contracts.length === 0) {
    return res
      .status(404)
      .json({ message: "No contracts found with the given parameters" });
  }
...
const updateContract = async (req, res) => {
  ...
  if (company_id) {
    if (isNaN(company_id)) {
      return res
        .status(403)
        .json({ message: "Company_id should be a number!" });
    }
    contract.company_id = company_id || contract.company_id;
  }
